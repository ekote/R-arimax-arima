...
deriv=function(u){
	...
	#        Z$a[] <- 0
	   ...
	   if (missing(io) & missing(xtransf) & missing(transfer)) {
	   	res = stats:::arima( 	x=x, order=order, seasonal=seasonal, xreg=xreg, include.mean=include.mean, 
					transform.pars=transform.pars, fixed=fixed, init=init, method=method, 
					n.cond=if(!missing(n.cond)) n.cond, optim.control=optim.control,kappa=kappa)
	      	res$call=match.call()
	      	res$aic=res$aic-2
	      	return(res)
	}
	    ...
	    Delta <- 1
			...
			# If xreg has some value than we crate xreg=data.frame with xreg values
		    if(!is.null(xreg)) xreg=data.frame(xreg)

			# If xreg has some value than xreg is MATRIX
		    if (!is.null(xreg))
			xreg = as.matrix(xreg)
		    xreg = cbind(xreg, Pm)
			# WE HAVE XREG OBJECT
			# Combine R Objects by Rows or Columns [http://www.inside-r.org/r-doc/base/cbind]
			...
			# If xreg is null than ncxreg is = 0
		    if (is.null(xreg)) {
			ncxreg <- 0
		    }
		    else {
			# If xreg is NOT null than
			nmxreg <- deparse(substitute(xreg))
			# nmreg - Turn unevaluated expressions into character strings.
			
			# if number of row of XREG is different than n (?)
			if (NROW(xreg) != n)
